...
const AdminSchools: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = {
      ...formData,
      classes: formData.classes.split(',').map(m => m.trim()).filter(m => m)
    };
    try {
      if (editingSchool) {
        await axios.put(`${API_URL}/schools/${editingSchool._id}`, data, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setSchools(schools.map(h => h._id === editingSchool._id ? { ...h, ...data } : h));
      } else {
      ...
      setFormData({ name: '', image: '', phone: '', address: '', googleMap: '', board: '', type: '', classes: '' });
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setSchools(schools.filter(h => h._id !== id));
    } catch (error) {
    ...
    <AdminLayout title="Schools">
      <div className="max-w-6xl mx-auto px-4 space-y-4 pt-6">
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 bg-gradient-to-br from-blue-600 to-indigo-600 text-white font-bold rounded-2xl shadow-lg hover:shadow-xl transition-all"
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">Board</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Classes (comma separated)</label>
                <input
                  type="text"
                  value={formData.classes}
                  onChange={(e) => setFormData({ ...formData, classes: e.target.value })}
                  className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:outline-none focus:border-blue-500"
                  placeholder="Nursery, LKG, UKG, Class 1 to 10"
                />
              </div>
              <button
                type="submit"
                className="w-full py-3 bg-gradient-to-br from-blue-600 to-indigo-600 text-white font-bold rounded-xl"
              >
        ...
        {loading ? (
          <div className="text-center py-20 text-slate-500 text-lg">Loading schools...</div>
        ) : schools.length === 0 ? (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-2">No Schools Yet</h3>
            <p className="text-slate-500">Add schools to display</p>
          </motion.div>
        ) : (
                  ...
                  <div>
                    ...
                    {school.phone && (
                      <div className="mt-2 flex items-center gap-2 text-slate-600">
                        ...
                        <a href={`tel:${school.phone}`} className="font-bold text-slate-800 hover:text-blue-600">
                          {school.phone}
                        </a>
                      </div>
                    )}
                    {school.board && (
                      <div className="mt-2 text-sm text-slate-600">
                        <span className="font-semibold">Board: {school.board}</span>
                      </div>
                    )}
                    {school.type && (
                      <div className="mt-1 text-sm text-slate-600">
                        <span className="font-semibold">Type: {school.type}</span>
                      </div>
                    )}
                    {school.address && (
                    ...
                    {school.classes?.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {school.classes.map((cls: string, i: number) => (
