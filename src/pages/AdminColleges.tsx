...
const AdminColleges: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = {
      ...formData,
      courses: formData.courses.split(',').map(m => m.trim()).filter(m => m)
    };
    try {
      if (editingCollege) {
        await axios.put(`${API_URL}/colleges/${editingCollege._id}`, data, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setColleges(colleges.map(h => h._id === editingCollege._id ? { ...h, ...data } : h));
      } else {
      ...
      setFormData({ name: '', image: '', phone: '', address: '', googleMap: '', university: '', type: '', courses: '' });
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setColleges(colleges.filter(h => h._id !== id));
    } catch (error) {
    ...
    <AdminLayout title="Colleges">
      <div className="max-w-6xl mx-auto px-4 space-y-4 pt-6">
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 bg-gradient-to-br from-emerald-600 to-green-600 text-white font-bold rounded-2xl shadow-lg hover:shadow-xl transition-all"
        >
        ...
        {showForm && (
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Courses (comma separated)</label>
                <input
                  type="text"
              ...
              <button
                type="submit"
                className="w-full py-3 bg-gradient-to-br from-emerald-600 to-green-600 text-white font-bold rounded-xl"
              >
        ...
        {loading ? (
          <div className="text-center py-20 text-slate-500 text-lg">Loading colleges...</div>
        ) : colleges.length === 0 ? (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-2">No Colleges Yet</h3>
            <p className="text-slate-500">Add colleges to display</p>
          </motion.div>
        ) : (
                  ...
                  <div>
                    ...
                    {college.phone && (
                      <div className="mt-2 flex items-center gap-2 text-slate-600">
                        ...
                        <a href={`tel:${college.phone}`} className="font-bold text-slate-800 hover:text-emerald-600">
                          {college.phone}
                        </a>
                      </div>
                    )}
                    {college.university && (
                      <div className="mt-2 text-sm text-slate-600">
                        <span className="font-semibold">University: {college.university}</span>
                      </div>
                    )}
                    {college.type && (
                      <div className="mt-1 text-sm text-slate-600">
                        <span className="font-semibold">Type: {college.type}</span>
                      </div>
                    )}
                    {college.address && (
