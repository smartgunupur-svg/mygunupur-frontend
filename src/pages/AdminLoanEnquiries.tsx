...
const AdminLoanEnquiries: React.FC = () => {
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      const token = localStorage.getItem('adminToken');
      await axios.put(`${API_URL}/loan-enquiries/${id}/status`, { status }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchEnquiries();
    } catch (error) {
    ...
    <div className="max-w-6xl mx-auto px-4 space-y-6 pt-6">
      ...
      {loading ? (
        ...
        >
          ...
          <p className="text-slate-500">No {activeTab !== 'all' ? statusConfig[activeTab as keyof typeof statusConfig].label.toLowerCase() : ''} loan enquiries yet</p>
        </motion.div>
      ) : (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="px-3 py-2 bg-slate-100 border border-slate-200 rounded-xl font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      >
